...
# Corona virus data here: https://www.kaggle.com/sudalairajkumar/novel-corona-virus-2019-dataset
# (direct download: https://www.kaggle.com/sudalairajkumar/novel-corona-virus-2019-dataset/download).
...
# 
# It comes under the form of a zip file (currently named `novel-corona-virus-2019-dataset.zip`
# with several `.csv` files in them. We use `py2store` (To install: `pip install py2store`.
# Project lives here: https://github.com/i2mint/py2store) to access and pre-prepare it.
# It allows us to not have to unzip the file and replace the older folder with it every time we download a new one.
...
def country_data_for_data_kind(data_sources=DFLT_DATA_SOURCES_DIR, kind='confirmed', skip_first_days=0, verbose=False):
...
def mk_and_save_covid_data(data_sources=DFLT_DATA_SOURCES_DIR,
    ...
    data_sources = get_data_sources(data_sources)
    for kind in kinds:
        mk_and_save_country_data_for_data_kind(data_sources, kind=kind, save_dirpath=save_dirpath,
                                               skip_first_days=skip_first_days, verbose=verbose)
